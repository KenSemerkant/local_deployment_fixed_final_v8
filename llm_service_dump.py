...
# ... (Pydantic models omitted for brevity, they are unchanged) ...
        ...
        def _update_step(step_name):
            if callback_url:
                try:
                    ...
                    requests.patch(callback_url, json={"step": step_name}, headers=headers, timeout=5)
                except Exception as e:
...
class OllamaLLMClient:
    ...
    def answer_question(self, document_path: str, question: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with get_openai_callback() as cb:
                result = qa({"query": question})
                print(f"DEBUG: Callback state: {cb}")
                print(f"DEBUG: Result keys: {result.keys()}")
                if "source_documents" in result:
                    print(f"DEBUG: Source docs count: {len(result['source_documents'])}")
                ...
                if cb.total_tokens == 0:
                    print("DEBUG: Callback captured 0 tokens. Checking result for usage info...")
                    # Sometimes it's in source_documents metadata or elsewhere? Unlikely for usage.
                    pass
...
def _update_step_callback(document_id: int, callback_url: str, step: str):
    try:
        ...
        if INTERNAL_API_KEY:
            headers["X-Internal-API-Key"] = INTERNAL_API_KEY
        requests.patch(f"{callback_url}/step", json={"step": step}, headers=headers)
    except Exception as e:
...
def ask_question(request: QuestionRequest):
    ...
    print(f"DEBUG: Token usage from LLM: {token_usage}")
